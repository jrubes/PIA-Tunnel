...
switch($_REQUEST['cmd']){
  case 'force_update':
    if( $_token->pval($_GET['token'], 'force operating system update') === true ){
      exec("bash -c \"{$settings['CMD_SUDO']} /usr/local/pia/system-update.sh &> /dev/null &\" &>/dev/null &");
  ...
  case 'run_pia_command':
    if( array_key_exists('pia-update', $_POST) === true ){
      ...
      $result = array();
      exec( $settings['CMD_SUDO']." /usr/local/pia/pia-update", $result);
      ...
      if( $_token->pval($_POST['token'], 'complete system reset') === true ){
        ...
        exec( $settings['CMD_SUDO']." /usr/local/pia/pia-update");

        //GUI access to reset-pia
        $result = array();
        exec( $settings['CMD_SUDO']." /usr/local/pia/reset-pia", $result);
        if( array_key_exists('0', $result) === true ){
