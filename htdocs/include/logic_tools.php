...
switch($_REQUEST['cmd']){
  case 'force_update':
    if( $_token->pval($_GET['token'], 'force operating system update') === true ){
      exec("bash -c \"/usr/local/bin/sudo /usr/local/pia/system-update.sh &> /dev/null &\" &>/dev/null &");
  ...
  case 'run_pia_command':
    if( array_key_exists('pia-update', $_POST) === true ){
      ...
      $result = array();
      exec("/usr/local/bin/sudo /usr/local/pia/pia-update", $result);
...
function disp_client_tools(){
  ...
  $ret = array();
  exec('/sbin/ip addr show '.$if_LAN.' | /usr/bin/grep -w "inet" | /usr/local/bin/gawk -F" " \'{print $2}\' | /usr/bin/cut -d/ -f1', $ret);
